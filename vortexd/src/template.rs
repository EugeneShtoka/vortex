...
/// Render a shell command template, substituting:
///
/// - `{{tasks.<id>.stdout}}` — captured stdout of a completed task
/// - `{{tasks.<id>.stderr}}` — captured stderr of a completed task
/// - `{{tasks.<id>.success}}` — `true` / `false`
/// - `{{tasks.<id>.exit_code}}` — integer exit code
/// - `{{env.<NAME>}}` — environment variable
/// - `{{globals.<key>}}` — value from the SQLite global store
///
/// Missing keys render as empty string (non-strict mode). Values are
/// not HTML-escaped so they are safe to embed in shell commands.
pub fn render(
    template: &str,
    ...
    let ctx = json!({ "tasks": tasks, "env": env, "globals": globals_val, "trigger": trigger_val });
...
mod tests {
    ...
    fn result(id: &str, stdout: &str, stderr: &str, success: bool, exit_code: i32) -> TaskResult {
        TaskResult { id: id.into(), stdout: stdout.into(), stderr: stderr.into(), exit_code, success }
    }
    ...
    fn plain_string_unchanged() {
        assert_eq!(render("echo hello", &HashMap::new(), &no_globals(), &no_params()).unwrap(), "echo hello");
    }
    ...
    fn task_stdout_substituted() {
        ...
        assert_eq!(
            render("echo {{tasks.step1.stdout}}", &rs, &no_globals(), &no_params()).unwrap(),
            "echo hello world\n"
        );
    }
    ...
    fn task_stderr_substituted() {
        ...
        assert_eq!(
            render("cat <<< {{tasks.step1.stderr}}", &rs, &no_globals(), &no_params()).unwrap(),
            "cat <<< oops\n"
        );
    }
    ...
    fn task_success_substituted() {
        ...
        let out = render("status={{tasks.build.success}}", &rs, &no_globals(), &no_params()).unwrap();
    ...
    fn task_exit_code_substituted() {
        ...
        let out = render("code={{tasks.build.exit_code}}", &rs, &no_globals(), &no_params()).unwrap();
    ...
    fn env_var_substituted() {
        ...
        let out = render("echo {{env.VORTEX_TEST_VAR}}", &HashMap::new(), &no_globals(), &no_params()).unwrap();
    ...
    fn missing_env_var_renders_empty() {
        ...
        let out =
            render("echo {{env.VORTEX_DEFINITELY_MISSING_VAR}}", &HashMap::new(), &no_globals(), &no_params())
                .unwrap();
        assert_eq!(out, "echo ");
    }
    ...
    fn globals_substituted() {
        ...
        let out = render("echo {{globals.deploy_count}}", &HashMap::new(), &g, &no_params()).unwrap();
    ...
    fn missing_global_renders_empty() {
        let out =
            render("echo {{globals.missing}}", &HashMap::new(), &no_globals(), &no_params()).unwrap();
        assert_eq!(out, "echo ");
    }
    ...
    fn ampersand_not_escaped() {
        ...
        let out = render("{{tasks.step.stdout}}", &rs, &no_globals(), &no_params()).unwrap();
    ...
    fn multiple_variables_in_one_template() {
        ...
        let out = render("cp {{tasks.build.stdout}} {{globals.dest}}", &rs, &g, &HashMap::new()).unwrap();
    ...
    fn trigger_param_substituted() {
        ...
        let out = render("echo {{trigger.msg}}", &HashMap::new(), &HashMap::new(), &p).unwrap();
        assert_eq!(out, "echo hello world");
    }

    #[test]
    fn missing_trigger_param_renders_empty() {
        let out = render("echo {{trigger.missing}}", &HashMap::new(), &HashMap::new(), &HashMap::new()).unwrap();
        assert_eq!(out, "echo ");
    }
    ...
    fn trigger_param_combined_with_task_output() {
        ...
        let out = render("cp {{tasks.step.stdout}} {{trigger.dest}}", &rs, &HashMap::new(), &p).unwrap();
